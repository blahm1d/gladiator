...
$nvram = Join-Path $out "mame-input-mcu-nvram"
$cfg = Join-Path $out "mame-input-mcu-cfg"
$output = Join-Path $out "mame-input-mcu-host.csv"
$stdout = Join-Path $out "mame-input-mcu-stdout.log"
$stderr = Join-Path $out "mame-input-mcu-stderr.log"
...
$arguments = @(
    ...
    "-skip_gameinfo",
    "-autoboot_delay", "0",
    "-autoboot_script", "sim/mame/gladiatr_input_mcu_host.lua",
...
if (-not $process.WaitForExit(120000)) {
    ...
    throw "Headless MAME input-MCU capture exceeded the bounded timeout."
}
...
if ($exitCode -ne 0) {
    throw "Headless MAME input-MCU capture failed. See $stderr"
}
...
if (-not $last -or [int] $last.frame -lt ($Frames - 2)) {
    throw "MAME input-MCU capture ended before the requested frame."
}
$cctl = @($rows | Where-Object { $_.domain -eq "T" }).Count
$ccpu = @($rows | Where-Object { $_.domain -eq "P" }).Count
if ($cctl -eq 0 -or $ccpu -eq 0) {
    throw "MAME input-MCU capture missed a required host domain."
}

Write-Host (
    "PASS input-MCU host capture through frame {0}: CCTL={1}, CCPU={2}" -f
    $last.frame, $cctl, $ccpu)
